 ...
 *
 */
typedef struct vl53lox_config_t {
...
bool init_vl53l0x(vl53lox_config_t* config) {
  /* gpio init */
  if (config->gpio_xshut != GPIO_NUM_MAX) {
    gpio_set_direction(config->gpio_xshut, GPIO_MODE_OUTPUT);
    gpio_set_level(config->gpio_xshut, 1);
  }
  if (config->gpio_gpio1 != GPIO_NUM_MAX) {
    ESP_ERROR_CHECK(gpio_set_direction(config->gpio_gpio1, GPIO_MODE_INPUT));
    ESP_ERROR_CHECK(gpio_set_pull_mode(config->gpio_gpio1, GPIO_PULLUP_ONLY));
    ESP_ERROR_CHECK(gpio_pullup_en(config->gpio_gpio1));
    ESP_ERROR_CHECK(gpio_set_intr_type(config->gpio_gpio1, GPIO_INTR_POSEDGE));
    ESP_ERROR_CHECK(gpio_install_isr_service(0));
    ESP_ERROR_CHECK(gpio_isr_handler_add(config->gpio_gpio1, gpio1_isr, this));
